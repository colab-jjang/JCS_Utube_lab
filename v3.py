# -*- coding: utf-8 -*-
# 📺 48시간 유튜브 숏츠 트렌드 대시보드 (정치·뉴스) — GLOBAL_STOPWORDS 통합판
...
def save_quota_used(value: int):
    QUOTA_FILE.write_text(json.dumps({"pt_date": _today_pt_str(), "used": int(value)}), encoding="utf-8")

def add_quota(cost: int):
    used = st.session_state.get("quota_used", 0) + int(cost)
    st.session_state["quota_used"] = used
    save_quota_used(used)
...
# ───────────────────────────────────────────────────────────────────────────────
# 공용 금지(문장 레벨) + 공용 불용어(토큰 레벨)
# ───────────────────────────────────────────────────────────────────────────────
# ❗문장 전체에서 걸러낼 표현(어디서든 공통 차단)
...
# 브랜드/매체명(유튜브에서 특히 많이 등장)
MEDIA_BRANDS = {
    "sbs","kbs","mbc","jtbc","tv조선","mbn","ytn","yonhap","yonhapnews",
    "조선일보","중앙일보","동아일보","한겨레","경향","연합뉴스"
}

# ✅ 전역 불용어(유튜브+트렌드 공통)
GLOBAL_STOPWORDS = {
    # 기능어/접속어
    *"그리고 그러나 그래서 또한 또는 및 먼저 지금 바로 매우 정말 그냥 너무 보다 라는 이런 저런 그런".split(),
    *"합니다 했다 했다가 하는 하고 하며 하면 대한 위해 에서 에게 에도 에는 으로 로 를 은 는 이 가 도 의 에 와 과".split(),
    # 뉴스상투어/군더더기
    *"속보 브리핑 단독 현장 영상 뉴스 기자 리포트 라이브 헤드라인 데스크 전체보기 더보기 전문 시작 사고 현장중계".split(),
    *"오늘 어제 금일 최근 방금 방금전 아침 오전 오후 밤 새벽".split(),
    *"관련 논란 논쟁 상황 사건 이슈 분석 전망 발표 발언 입장 보도 공개 사진 채널 구독 news".split(),
    # 지명/기관(너무 일반적)
    *"서울 한국 국내 해외 정부 여당 야당 당국 위원장 장관 대통령 총리 국회 정치 대통령실 대법원 특검".split(),
    # URL/플랫폼
...
# ───────────────────────────────────────────────────────────────────────────────
# 유튜브: 키워드 토큰화 + 집계 (GLOBAL_STOPWORDS 사용)
# ───────────────────────────────────────────────────────────────────────────────
def tokenize_ko_en(text: str):
    ...
    for t in raw:
        ...
        if t in GLOBAL_STOPWORDS or t in MEDIA_BRANDS or len(t) < 2:
            continue
        if re.fullmatch(r"[a-z]+", t) and len(t) <= 2:
            continue
        if t.endswith("tv") and len(t) > 2:
            t = t[:-2]
            if t in GLOBAL_STOPWORDS or len(t) < 2:
                continue
        out.append(t)
    return out
...
# ───────────────────────────────────────────────────────────────────────────────
# 트렌드(네이버/구글) 전용 규칙 — 문장형 헤드라인 정제 + 유사구 병합
# ───────────────────────────────────────────────────────────────────────────────
...
def _tok_line_for_trends(s: str) -> list[str]:
    ...
    for t in toks:
        ...
        if t in GLOBAL_STOPWORDS: continue
...
def _is_bad_phrase(ph: str) -> bool:
    ...
    if len(ws)==2:
        w0 = ws[0]; w1 = last
        if (w0 in GLOBAL_STOPWORDS or w1 in GLOBAL_STOPWORDS or
...
def _phrase_signature(ph: str) -> tuple:
    ...
    sig = [
        w for w in ws
        if w and w not in GLOBAL_STOPWORDS and
...
def google_trends_top(debug_log: bool = False, source_mode: str = "auto"):
    ...
    if source_mode == "naver":
        kws, src = _fetch_trends_naver(add);  return kws, src, logs
...
with right:
    ...
    if g_kw:
        ...
        st.info("선택한 소스에서 트렌드 키워드를 가져오지 못했습니다. (모드를 바꾸거나 디버그로 로그 확인)")
...
- 쿼터 비용(추정): `search.list = 100/호출`, `videos.list = 1/호출(50개 단위)`.
- 캐시 TTL을 길게 설정하면 쿼터 사용량을 줄일 수 있습니다.
- 트렌드 소스는 *구글(RSS→HTML)* 실패 시 *네이버 인기뉴스*로 자동 대체합니다.
""")
