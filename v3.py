...
COMMON_STOPWORDS = {
    ...
    # 네가 요청한 공용 블랙리스트
    "구독","정치","대통령실","채널","news","대법원","특검",
    # 방송사/매체 상수
    "sbs","kbs","mbc","jtbc","tv조선","mbn","연합뉴스",
    # 자주 뜨는 군더더기
    "시작","사고","전문","사진"
}
...
def _contains_common_banned(s: str) -> bool:
    s = s.lower()
    if COMMON_BANNED_PAT.search(s):
        return True
    if TEMPORAL_BAD_PAT.search(s):
        return True
    return False
...
def top_keywords_from_df(df: pd.DataFrame, topk:int=10):
    ...
    words = [w for w,_ in items[:topk*3]]  # 넉넉히 뽑고
    words = _postprocess_phrases(words, topk=topk)  # 유사구 병합/필터
    # count 재구성(시각화용): 남은 단어만 카운트
    cnt2 = Counter()
    for line in corpus:
...
_POSTPOSITION_SUFFIXES = ("으로","로","에게","에서","보다","까지","부터","만","조차","라도","처럼","뿐","께","에","와","과","랑","하고","밖에","이라","라","이라도","은","는","이","가","채")
...
PHRASE_SIM_STOP = {
    ...
    "사실","상황","사건","문제","논란","논쟁","발언","입장","보도","당시","현장","추정","추락","폭행","체포","구속","달 만에"
}
