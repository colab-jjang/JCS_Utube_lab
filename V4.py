...
def fmt_hms(seconds: int | None) -> str:
    if seconds is None:
        return ""
    h = seconds // 3600
    m = (seconds % 3600) // 60
...
def api_get(url: str, params: dict, cost: int) -> dict:
...
def search_ids(keyword: str, max_pages: int = 1) -> list[str]:
...
def fetch_details(video_ids: list[str]) -> list[dict]:
    out: list[dict] = []
...
def make_dataframe(keyword: str, max_pages: int = 1) -> pd.DataFrame:
...
# ====== UI ======
st.set_page_config(page_title="YouTube Shorts 24h Finder", page_icon="📺", layout="wide")
st.title("📺 24시간 이내 업로드된 YouTube Shorts 찾기 (KR)")

if not API_KEY:
    st.error("⚠️ API 키가 설정되지 않았습니다. 좌측 메뉴(▶) > Settings > Secrets 에 YOUTUBE_API_KEY를 추가하세요.")
    st.stop()

# 세션 상태에 쿼터 카운터 준비 (파일과 동기화)
...
with st.sidebar:
    ...
    st.caption("범위: 현재 시각(KST) 기준 **지난 24시간**")
...
with quota_col2:
    st.metric("남은 쿼터(추정)", value=f"{remaining:,}",
              delta=f"리셋까지 {remaining_td}".replace("days", "일").replace("day", "일"))
st.caption("※ 일일 쿼터는 PT 자정(한국시간 다음날 16~17시)에 리셋")
...
if run_btn:
    if not keyword.strip():
        ...
        st.download_button(
            "CSV 다운로드",
            data=csv_bytes,
            file_name=f"shorts_24h_{keyword}.csv",
            mime="text/csv",
        )

        st.info(f"이번 실행으로 추정 사용량: search.list {100 * int(max_pages)} + videos.list 1")
